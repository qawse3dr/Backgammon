...
public class Die
{
    ...
    {   // property
        ...
        private set
        {        // this should only be set once when object is instantiated
            if (0 <= value)
            {  // ensure that the number of die is non-negative
                _numDie = value;
                Logger.Debug(
    ...
    */
    public void Roll()
    {
        ...
        if (Rolls.All(o => o == Rolls[0]))
        {  // all die roll to the same values (i.e. doubles)
            Doubles(Rolls[0]);
            _doubles = true;
        }
